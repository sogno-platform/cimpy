...
import cimpy
from cimpy.cgmes_v3_0.Base import short_profile_name

logging.basicConfig(filename="Test_import.log", level=logging.INFO, filemode="w")

example_dir = Path(
    os.path.join(
        os.path.dirname(__file__), "../cimpy/examples/sampledata/CIM_LV_Simbench_Grid"
    )
).resolve()
...
def test_import():
    ...
    imported_result = cimpy.cim_import(test_files, "cgmes_v3_0")

    import_resolved = cimpy.cimexport._get_class_attributes_with_references(
        imported_result, "cgmes_v3_0"
    )

    ref_dict_path = Path(
        os.path.join(os.path.dirname(__file__), "CIM_v3_import_reference.p1")
    )
