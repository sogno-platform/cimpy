...
from cimpy.cgmes_v3_0.Base import Profile
import logging
import sys
from cimpy.cgmes_v3_0.Base import Base
    ...
    for key in attr_dict:
        ...
        if isinstance(attr_dict[key], list):  # many
            ...
            for elem in attr_dict[key]:
                if issubclass(type(elem), base_class):
                    ...
                    if not hasattr(elem, "mRID"):
                        ...
                        if UUID == "%":
                            logger.warning(
                                "Object of type {} not found as reference for object"
                                " with UUID {}.".format(elem.__class__.__name__, mRID)
                            )
                    else:
                        UUID = "%" + elem.mRID
                    ...
                    logger.warning(
                        "Reference object not subclass of Base class for object with"
                        " UUID {}.".format(mRID)
                    )
            if len(array) == 1:
            ...
            if not hasattr(attr_dict[key], "mRID"):
                ...
                if UUID == "%":
                    logger.warning(
                        "Object of type {} not found as reference for object with"
                        " UUID {}.".format(attr_dict[key].__class__.__name__, mRID)
                    )
            else:
        ...
        if "class" in serializationProfile.keys():
            ...
            if Profile[serializationProfile["class"]] in activeProfileList:
                ...
                if (
                    ...
                    logger.warning(
                        "Class {} was read from profile {} but this profile is not"
                        " possible for this class".format(
                            klass["name"], serializationProfile["class"]
                        )
                    )
            else:
                logger.info(
                    "Class {} was read from profile {} but this profile is not active"
                    " for the export. Usedefault profile from possibleProfileList."
            ...
            if klass["name"] in possibleProfileList.keys():
                if "class" in possibleProfileList[klass["name"]].keys():
                    ...
                    if class_serializationProfile == "":
                        ...
                        logger.warning(
                            "All possible export profiles for class {} not active. Skip"
                            " class for export.".format(klass["name"])
                        )
                        continue
                else:
                    logger.warning(
                    ...
                    if attribute_class in possibleProfileList.keys():
                        if (
                            ...
                            if attribute_serializationProfile == "":
                                ...
                                logger.warning(
                                    "All possible export profiles for attribute {}.{}"
                                    " of class {} not active. Skip attribute for"
                                    " export.".format(
                            ...
                            logger.warning(
                                "Attribute {}.{} of class {} has no profile to"
                                " export to.".format(
                                    attribute_class, attribute_name, klass["name"]
                                )
                            )
                        ...
                        logger.warning(
                            "The class {} for attribute {} is not in the"
                            " possibleProfileList".format(
...
def cim_export(import_result, file_name, version, activeProfileList):
    ...
    for profile in profile_list:
        ...
        if not os.path.exists(full_file_name):
            ...
            logger.error(
                "File {} already exists. Delete file or change file name to serialize"
                " CGMES classes.".format(full_file_name)
            )
            print(
                "[ERROR:] File {} already exists. Delete file or change file name to"
                " serialize CGMES classes.".format(full_file_name),
                file=sys.stderr,
            )
            exit(-1)
