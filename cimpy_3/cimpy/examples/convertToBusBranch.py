...
logging.basicConfig(
    filename="Convert_to_Bus_Branch.log", level=logging.INFO, filemode="w"
)
...
if "cimexamples.py" in str(__file__):
    sample_folder = (
        example / "examples" / "sampledata" / "Sample_Grid_Switches" / "Node-Breaker"
    )
else:
...
cimpy.cim_export(
    import_result, "Bus_Branch_Converted", "cgmes_v2_4_15", activeProfileList
)
