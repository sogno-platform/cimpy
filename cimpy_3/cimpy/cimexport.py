...
def cim_export(import_result, file_name, version, activeProfileList):
    ...
    for profile in profile_list:
        ...
        if not os.path.exists(full_path):
            ...
            logger.error(
                "File {} already exists in path {}. Delete file or change file name to"
                " serialize CGMES classes.".format(full_file_name, cwd)
            )
            print(
                "[ERROR:] File {} already exists in path {}. Delete file or change file"
                " name to serialize CGMES classes.".format(full_file_name, cwd),
                file=sys.stderr,
            )
            os.chdir(cwd)
            exit(-1)
