...
class OverexcLimIEEE(OverexcitationLimiterDynamics):
	...
	:itfpu: OEL timed field current limiter pickup level (I).  Typical Value = 1.05. Default: 
	:ifdmax: OEL instantaneous field current limit (I).  Typical Value = 1.5. Default: 
	:ifdlim: OEL timed field current limit (I).  Typical Value = 1.05. Default: 
	:hyst: OEL pickup/drop-out hysteresis (HYST).  Typical Value = 0.03. Default: 
	:kcd: OEL cooldown gain (K).  Typical Value = 1. Default: 
	:kramp: OEL ramped limit rate (K).  Unit = PU/sec.  Typical Value = 10. Default: 
		'''
	...
	def __init__(self, itfpu = , ifdmax = , ifdlim = , hyst = , kcd = , kramp = ,  *args, **kw_args):
