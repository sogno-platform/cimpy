...
class OverexcLimIEEE(OverexcitationLimiterDynamics):
	...
	:itfpu: OEL timed field current limiter pickup level (<i>I</i><i><sub>TFPU</sub></i>).  Typical value = 1,05. Default: 0.0
	:ifdmax: OEL instantaneous field current limit (<i>I</i><i><sub>FDMAX</sub></i>).  Typical value = 1,5. Default: 0.0
	:ifdlim: OEL timed field current limit (<i>I</i><i><sub>FDLIM</sub></i>).  Typical value = 1,05. Default: 0.0
	:hyst: OEL pickup/drop-out hysteresis (<i>HYST</i>).  Typical value = 0,03. Default: 0.0
	:kcd: OEL cooldown gain (<i>K</i><i><sub>CD</sub></i>).  Typical value = 1. Default: 0.0
	:kramp: OEL ramped limit rate (<i>K</i><i><sub>RAMP</sub></i>).  Unit = PU / s.  Typical value = 10. Default: 0.0
		'''
