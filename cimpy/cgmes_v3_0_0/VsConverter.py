...
class VsConverter(ACDCConverter):
	...
	possibleProfileList = {'class': [cgmesProfile.DY.value, cgmesProfile.SSH.value, cgmesProfile.EQ.value, cgmesProfile.SV.value, ],
						'VSCDynamics': [cgmesProfile.DY.value, ],
						...
						'targetPowerFactorPcc': [cgmesProfile.SSH.value, ],
						'targetPhasePcc': [cgmesProfile.SSH.value, ],
						'targetPWMfactor': [cgmesProfile.SSH.value, ],
						'CapabilityCurve': [cgmesProfile.EQ.value, ],
						'maxModulationIndex': [cgmesProfile.EQ.value, ],
						'delta': [cgmesProfile.SV.value, ],
						'uv': [cgmesProfile.SV.value, ],
						 }
	...
	def __init__(self, VSCDynamics = None, droop = 0.0, droopCompensation = 0.0, pPccControl = None, qPccControl = None, qShare = 0.0, targetQpcc = 0.0, targetUpcc = 0.0, targetPowerFactorPcc = 0.0, targetPhasePcc = 0.0, targetPWMfactor = 0.0, CapabilityCurve = None, maxModulationIndex = 0.0, delta = 0.0, uv = 0.0,  *args, **kw_args):
